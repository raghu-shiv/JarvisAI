...
export function PromptPanel({
  ...
  return (
    <>
      <button
        className={`fixed inset-0 z-40 bg-slate-950/30 transition-opacity ${isOpen ? "opacity-100" : "pointer-events-none opacity-0"}`}
        type="button"
        aria-label="Close prompt library"
        onClick={onClose}
      />
      <aside
        className={`fixed inset-y-0 right-0 z-50 flex w-[min(100vw,380px)] flex-col border-l border-slate-200 bg-white shadow-2xl transition-transform duration-200 ease-out ${
        ...
        <div className="flex h-16 items-center justify-between gap-3 border-b border-slate-200 px-4">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <div className="truncate text-sm font-semibold text-slate-950">Prompt Library</div>
              <div className="text-xs text-slate-500">{prompts.length} templates</div>
            </div>
          </div>
          <button className="rounded-md p-2 text-slate-500 hover:bg-slate-100 hover:text-slate-950" type="button" onClick={onClose} aria-label="Close prompt library" title="Close prompt library">
            <X size={18} />
          </button>
        </div>
